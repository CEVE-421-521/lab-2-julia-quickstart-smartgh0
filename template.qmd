...
author: "SOLOMON NYAMEKYE <sn66>" # YOU SHOULD EDIT THIS
date: 2024-01-19
week: 2
categories: [Lab]
...
typeof(DataFrame.(annual_data)) # should be a vector of DataFrames
```
...
#subset DataFrame using the range
df_month = @subset(df, t_start .<= :datetime .<= t_end)
#plotting selected data
plot(
    df_month.datetime,
    df_month.lsl;
    title ="Hourly Water levels on March 2020 at Sewells Point, VA",
    ylabel ="Water Level",
    label =false,
)
```
...
```{julia}
# create new column for year from DataFrame
df[!, :year] = Dates.year.(df.datetime)
dropmissing!(df, :lsl)
df_byyear = groupby(df, :year)
df_climatologyyear = combine(df_byyear, :lsl => mean => :lsl_avg);

#plotting new data
plot(
    df_climatologyyear.year,
    df_climatologyyear.lsl_avg;
    xlabel="Year",
...
```{julia}
# create new column for day from DataFrame
df[!, :dayofyear] = Dates.dayofyear.(df.datetime)
dropmissing!(df, :lsl)
df_bydayofyear = groupby(df, :dayofyear)
df_climatologydayofyear = combine(df_bydayofyear, :lsl => mean => :lsl_avg);
#plotting new data
plot(
    df_climatologydayofyear.dayofyear,
    df_climatologydayofyear.lsl_avg;
    xlabel="Day of Year",
    ylabel="Average Water level",
    linewidth=3,
    label= false,
